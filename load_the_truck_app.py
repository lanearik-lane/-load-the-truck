...
if not GAME_SLATE:
    ...
    for code, sides in DEMO_PITCHERS.items():
        PITCHERS[code] = {
            side: {"id": None, "name": name, "hand": hand} for side, (name, hand) in sides.items()
        }
    live_mode_active = False

default_game = params.get("game", f"{GAME_SLATE[0]['away']}-{GAME_SLATE[0]['home']}")
game_codes = [f"{g['away']}-{g['home']}" for g in GAME_SLATE]
...
if len(game_codes) > 1:
    picked = st.selectbox(
        ...
        format_func=lambda c: f"{c.split('-')[0]} @ {c.split('-')[1]}",
...
game_info = next(g for g in GAME_SLATE if f"{g['away']}-{g['home']}" == game)
